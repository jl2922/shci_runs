...
def plot_shci():
    ...
    f = interp1d(rs, es, kind='cubic')
    x_fit = np.linspace(1.5, 3.25, num=500)
    curve_3z_12e, = plt.plot(x_fit, f(x_fit), color='green', linestyle='solid')
    plt.plot(rs, es, color='green', marker='o', linestyle='none', alpha=0.7)

    es = []
    e_atom = -1049.760354
    df = pd.read_csv('3z_12e_HFC/energies.csv')
    es = (df['energy'].values - 2 * e_atom) * HA2EV
    f = interp1d(rs, es, kind='cubic')
    x_fit = np.linspace(1.5, 3.25, num=500)
    curve_3z_12e_hfc, = plt.plot(x_fit, f(x_fit), color='orange', linestyle='solid')
    ...
    return curve_3z_12e, curve_3z_12e_hfc
...
def plot():
    ...
    curve_3z_12e, curve_3z_12e_hfc = plot_shci()
    #curve_uhf, curve_ccsd, curve_ccsdt = plot_hfcc()

    plt.xlabel('Bond Length ($\AA$)')
    plt.ylabel('Atomization Energy (eV)')
    plt.title('Cr$_2$ Potential Energy Curve with 12e and cc-pVTZ')
    curves = (curve_3z_12e_hfc, curve_3z_12e, curve_experiment)
    labels = ('SHCI with HF Core Orbitals', 'SHCI with CAS Core Orbitals', 'Experiment')
    plt.legend(curves, labels)
    ax = plt.gca()
    ax.ticklabel_format(useOffset=False)
    plt.tight_layout()
    plt.grid(True, ls=':')
    plt.savefig('cashf.png', format='png', dpi=300)
    plt.show()
